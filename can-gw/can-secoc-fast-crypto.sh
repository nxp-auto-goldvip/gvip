...
# Default settings for hardware crypto: Tx frame with ID 68 on can0 , Rx frame with ID 70 on can1 , DLC 64 , 10 second period, 2ms gap between frames
common_params=(-g 2 -s 64 -t can0 -r can1 -i 68 -o 70 -l 10)
...
if ! secure_payload=$("${SCRIPTS_DIR}"/gen_secure_payload.sh -i "${data_id_hw}" -k "${mac_key}" -D "${authentic_payload}"); then
    echo "Secure payload generation failed."
    echo "${secure_payload}"
    exit 1
fi

echo "Simulate secured traffic using cryptographic primitives accelerated by HSE:"
